...
function rsync_mobileimport() {
    rsync -razutAXFH --progress ${HOME}/Pictures/Mobile-Import/ /Volumes/phlpptmpl/Photography/
}

# Find all MATLAB m-files and convert them to unix line ending
function mdos2unix() {
  if [ -z "${1}" ] ;then
    CWD=$(pwd)
  else
  ...
  find ${CWD} -type f -name "*.m" -not \( -path "*/worktree/*" -or -path "*/.git/*" -or -path "*/_minted-*" \) -exec dos2unix {} \;
}

# A quicker function that wraps around a custom `find` executing `dos2unix` with some exclusion masks
function d2u() {
  if [ -z "${1}" ] ;then
    CWD=$(pwd)
  else
...
function gmbw() {
  CWD=$(pwd)
  cd "${CWD}/worktree/master" && git merge develop && cd ${CWD}
}

function isodate() {
  date -u '+%Y%m%dT%k%M%SZ'
